...
void ParameterInfo::upscale_mask(cv::Mat& mask)
{
	...
	cv::resize(act_ds_mask, mask_mtx_us, tsize, (1 / ratio), (1 / ratio),CV_INTER_NN);
...
void ParameterInfo::draw_bounding_boxes()
{
	...
	cvFindContours(&bin_mask, storage, &seq, sizeof(CvContour),
	...
	const std::vector<std::string> result)
{
	...
	else if (in_data_format == type_video)
	{
		...
		vidfile >> first_img;
		cout << vidfile.get(cv::VideoCaptureProperties::CAP_PROP_FRAME_COUNT) << endl; //CV_CAP_PROP_FRAME_COUNT
		sequence_len = vidfile.get(cv::VideoCaptureProperties::CAP_PROP_FRAME_COUNT);
